...
describe('Fiel request builder', () => {
    ...
    test('query with invalid start date', () => {
        ...
        expect(() => requestBuilder.query(invalideDate, valideDate, requestBuilder.USE_SIGNER, '', 'CFDI'))
    ...
    test('query with invalid end date', () => {
        ...
        expect(() => requestBuilder.query(valideDate, invalideDate, requestBuilder.USE_SIGNER, '', 'CFDI'))
    ...
    test('query with start greater than end', () => {
        ...
        expect(() => requestBuilder.query(upper, lower, requestBuilder.USE_SIGNER, '', 'CFDI'))
            .toThrow('The period start "2019-01-01T00:00:01" is greater than end "2019-01-01T00:00:00"');
    });
    ...
    test('query with empty issuer receiver', () => {
        ...
        expect(() => requestBuilder.query(date, date, '', '', requestType))
            .toThrow('The RFC issuer and RFC receiver are empty');
    });
    ...
    test('query with issuer receiver not signer', () => {
        ...
        expect(() => requestBuilder.query(date, date, 'FOO', 'BAR', requestType))
            .toThrow('The RFC "EKU9003173C9" must be the issuer "FOO" or receiver "BAR"');
    });
    ...
    test('verify', async () => {
        ...
        const xmlSecVeritifaction = await (new EnvelopSignatureVerifier()).verify(
            requestBody,
            'http://DescargaMasivaTerceros.sat.gob.mx', 'VerificaSolicitudDescarga'
        );
        // TODO: check this verification.
        expect(xmlSecVeritifaction).toBeTruthy();
    });
    ...
    test('download', async () => {
        ...
        const xmlSecVeritifaction = await (new EnvelopSignatureVerifier()).verify(
            requestBody,
            'http://DescargaMasivaTerceros.sat.gob.mx', 'PeticionDescargaMasivaTercerosEntrada'
        );
        // TODO: check this verification.
        expect(xmlSecVeritifaction).toBeTruthy();
    });
