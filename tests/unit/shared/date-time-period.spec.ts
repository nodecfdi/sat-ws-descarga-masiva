...
describe('date time period', () => {
    ...
    test('create with string values', () => {
        ...
        const dateTimePeriod = DateTimePeriod.createFromValues(
            startValue,
            endValue
        );

        expect(dateTimePeriod.getStart()).toStrictEqual(
            DateTime.create(startValue)
        );
        expect(dateTimePeriod.getEnd()).toStrictEqual(
            DateTime.create(endValue)
        );
    });
    ...
    test('create with end date time less  than start date time', () => {
        ...
        expect(() => DateTimePeriod.create(start, end)).toThrow(
