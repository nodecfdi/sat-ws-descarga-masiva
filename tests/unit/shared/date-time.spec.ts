...
describe('date time', () => {
    ...
    test('create using timezon zulu', () => {
        ...
        expect(date.formateDefaultTimeZone()).toBe(
            '2019-01-13T22:23:24.000-06:00'
        );
    });
    ...
    test('create date time with invalid string value', () => {
        expect(() => DateTime.create('foo')).toThrow(
            'Unable to create a Datetime("foo")'
        );
    });
    ...
    test('create date time with invalid argument', () => {
        ...
        expect(() => DateTime.create(invalidDate)).toThrow(
